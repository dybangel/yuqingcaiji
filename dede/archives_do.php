...
<tr height='32'>
    <td width="80" class='bline'>&nbsp;所属区域：</td>
    <td class='bline'>
        <?php
         $dsql->SetQuery("SELECT * FROM `#@__arcatt` where att  like '00%' ORDER BY sortid ASC");
         $dsql->Execute();
        //    $typeOptions = GetOptionList($arcRow['typeid'],$cuserLogin->getUserChannel(), $arcRow['channel']);
            echo "<select name='flags[]' style='width:70%'>\r\n";
           echo "<option value='0' selected>请选择区域...</option>\r\n";  
          while($trow=$dsql->GetObject()){
           echo  "<option value='{$trow->att}' selected>{$trow->attname}</option>\r\n";
            }
            // if($arcRow["typeid"]=="0") echo "<option value='0' selected>请选择栏目...</option>\r\n";
            // echo $typeOptions;
            echo "</select>";
...
<tr height='32'>
    <td width="80" class='bline'>&nbsp;所属行业：</td>
    <td class='bline'>
        <?php
         $dsql->SetQuery("SELECT * FROM `#@__arcatt` where att  like 'h%' ORDER BY sortid ASC");
         $dsql->Execute();
        //    $typeOptions = GetOptionList($arcRow['typeid'],$cuserLogin->getUserChannel(), $arcRow['channel']);
            echo "<select name='flags[]' style='width:70%'>\r\n";
           echo "<option value='0' selected>请选择区域...</option>\r\n";  
...
<tr height='28'>
    <td width="80" class='bline'>&nbsp;级 别：</td>
    <td class='bline'>
    <input type='hidden' name='oldflag' value='<?php echo $arcRow['flag']; ?>' />
    <?php
    $dsql->SetQuery("SELECT * FROM `#@__arcatt` where att like 'e%' ORDER BY sortid ASC");
    $dsql->Execute();
    while($trow = $dsql->GetObject())
    {
        if($trow->att=='j' || $trow->att=='p') continue;
                  if(preg_match("#".$trow->att."#", $arcRow['flag']))
                  echo "<input class='np' type='radio' name='flags[]' id='flags{$trow->att}' value='{$trow->att}' checked='checked' />{$trow->attname}.{$trow->att}";
        else
                  echo "<input class='np' type='radio' name='flags[]' id='flags{$trow->att}' value='{$trow->att}' />{$trow->attname}.{$trow->att}";
    }
    ?>
    </td>
</tr>
