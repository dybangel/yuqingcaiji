...
class ListView
{
    ...
    function Display()
    {
        ...
        //echo "<pre>";
        $t_typeid=$this->dtp->refObj->TypeID;
        if($t_typeid=='-1'){
         global $dsql;	
           $sql='select count(*) as c from dede58_archives';
           $count=$dsql->GetOne($sql);//将SQL查询语句格式化
           //页数，每页10条               
					$this->dtp->refObj->TotalPage=ceil($count['c']/10);
					//记录数
					$this->dtp->refObj->TotalResult=$count['c'];
  //var_dump($this->dtp->refObj->TypeLink->dsql->queryString);
 $this->dtp->refObj->TypeLink->dsql->queryString ="SELECT COUNT(*) AS dd FROM `dede58_arctiny` arc WHERE  arc.arcrank > -1  AND (  arc.typeid >-1  OR CONCAT(',', arc.typeid2, ',') like  '%,-1,%'   ) LIMIT 0,1;";  
  //var_dump($this->dtp->refObj->addSql);
 $this->dtp->refObj->addSql ="arc.arcrank > -1  AND (  arc.typeid > -1  OR CONCAT(',', arc.typeid2, ',') like  '%,-1,%'   )";
  //var_dump($this->dtp->refObj->dsql->queryString);//="";
 $this->dtp->refObj->dsql->queryString ="SELECT COUNT(*) AS dd FROM `dede58_arctiny` arc WHERE  arc.arcrank > -1  AND (  arc.typeid > -1  OR CONCAT(',', arc.typeid2, ',') like  '%,-1,%'   ) LIMIT 0,1;";
      //var_dump($this->dtp->refObj);
  //    die();
         }
//var_dump($this->dtp->refObj->TypeID);die();     
//var_dump($this->dtp->refObj->TypeLink->dsql->queryString);die();   
//var_dump($this->dtp->refObj->addSql);die();   
