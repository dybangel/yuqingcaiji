...
else if($dopost=='save')
{
    ...
    if(empty($typeid))
    {
        layerMsg("请指定文档的栏目！","-1",0);
        exit();
    }
    if(empty($channelid))
    {
        layerMsg("文档为非指定的类型，请检查你发布内容的表单是否合法！","-1",0);
        exit();
    }
    if(!CheckChannel($typeid,$channelid))
    {
        layerMsg("你所选择的栏目与当前模型不相符，请选择白色的选项！","-1",0);
        exit();
    }
    ...
    if(empty($arcID))
    {
        layerMsg("无法获得主键，因此无法进行后续操作！","-1",0);
        exit();
    }
    if(trim($title) == '')
    {
    ...
    if(!$dsql->ExecuteNoneQuery($query))
    {
        ...
        layerMsg("把数据保存到数据库主表 `#@__archives` 时出错，请把相关信息提交给DedeCms官方。".str_replace('"','',$gerr),"-1",0);
        exit();
    }
    ...
    if(empty($addtable))
    {
        ...
        $dsql->ExecuteNoneQuery("DELETE FROM `#@__arctiny` WHERE id='$arcID'");
        layerMsg("没找到当前模型[{$channelid}]的主表信息，无法完成操作！。","-1",0);
        exit();
    }
    $useip = GetIP();
    ...
    if(!$dsql->ExecuteNoneQuery($query))
    {
        ...
        $dsql->ExecuteNoneQuery("Delete From `#@__archives` where id='$arcID'");
        $dsql->ExecuteNoneQuery("Delete From `#@__arctiny` where id='$arcID'");
        layerMsg("把数据保存到数据库附加表 `{$addtable}` 时出错，请把相关信息提交给DedeCms官方。".str_replace('"','',$gerr),"-1",0);
        exit();
    }
    //生成HTML
